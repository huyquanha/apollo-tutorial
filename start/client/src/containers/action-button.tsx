...
import * as CancelTripTypes from './__generated__/cancel';
...
const CancelTripButton: React.FC<ActionButtonProps> = ({ id }) => {
  const [mutate, { loading, error }] = useMutation<
    CancelTripTypes.cancel,
    CancelTripTypes.cancelVariables
  >(CANCEL_TRIP, {
    variables: { launchId: id! },
    update(cache, { data: { cancelTrip } }: any) {
      // update the user's cached list of trips to remove
      // the trip that was just canceled
      ...
      cache.modify({
        ...
        fields: {
          trips(existingTrips, { readField }) {
            // filter out the canceled launch frome the trips list
            return existingTrips.filter(
              (tripRef: Reference) => readField('id', tripRef) !== launch.id
            );
          },
        },
      });
